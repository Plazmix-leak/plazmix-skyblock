...
public class GlowingMainBlockUpgrade implements IslandUpgrade {

    public static final int UPGRADE_PRICE = 15_000;
    ...
    public ItemStack getItemStack(@NonNull Player player) {
        ...
        int upgradeCost = UPGRADE_PRICE;

        ChatColor chatColor = (plazmixUser.getCoins() >= upgradeCost ? ChatColor.YELLOW : ChatColor.RED);


        ItemUtil.ItemBuilder itemBuilder = ItemUtil.newBuilder(Material.JACK_O_LANTERN);
        itemBuilder.setName(chatColor + "Подсветка главного блока");
    ...
    public void onItemAction(@NonNull Player player, @NonNull SkyIsland skyIsland) {
        ...
        int upgradeLevel = skyIsland.getUpgradeLevel(this);
        int upgradeCost = upgradeLevel * UPGRADE_PRICE;
        ...
        skyIsland.alertMessage(true, "§6§lOneBlock §8:: §fПодсветка главного блока была улучшена до §e" + upgradeLevel + " уровня" + " §7(" + ((upgradeLevel * 30) + "x" + (upgradeLevel * 30)) + ")");
